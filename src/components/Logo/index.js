...
const Logo = () => {
  ...
  const createSprinkles = () => {
    ...
    const newSprinkles = Array.from({ length: 6 }).map((_, i) => ({
      id: Date.now() + i,
      x: Math.random() * window.innerWidth, // full screen X
      emoji: flowers[Math.floor(Math.random() * flowers.length)],
    }));

    setSprinkles((prev) => [...prev, ...newSprinkles]);

    // remove old ones after 3s
    setTimeout(() => {
      setSprinkles((prev) =>
        prev.filter((s) => Date.now() - s.id < 2800)
      );
    }, 3000);
  };
  ...
  const handleMouseEnter = () => {
    if (!intervalRef.current) {
      createSprinkles();
      intervalRef.current = setInterval(createSprinkles, 500);
    }
  };
  ...
  return (
    <>
      ...
      {/* FULL SCREEN FLOWERS */}
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none overflow-visible z-[9999]">
        <AnimatePresence>
          {sprinkles.map((s) => (
            <motion.div
              key={s.id}
